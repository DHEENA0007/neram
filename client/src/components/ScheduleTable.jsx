...
function ActivityBadge({ activity, lang }) {
  ...
  return (
    <span className={`inline-block px-3 py-1 rounded-full text-xs font-black uppercase tracking-wide ${cls}`}>
      {n(activity, lang)}
    </span>
  );
}
...
function StrengthBadge({ strength }) {
  ...
  return (
    <span className={`inline-block px-3 py-1.5 rounded-lg text-xs font-black tabular-nums ${cls}`}>
      {strength}%
    </span>
  );
}
    ...
    <section className="glass-card overflow-hidden p-0 border-none shadow-premium animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="hidden lg:block overflow-x-auto">
        <table className="w-full text-left border-collapse" style={{ tableLayout: 'fixed' }}>
          ...
          <thead>
            <tr className={`text-xs font-black uppercase tracking-widest bg-slate-50/60 border-b border-slate-100 ${thTxt}`}>
              ...
              <th className="px-5 py-5 text-right">{c.strength}</th>
            </tr>
          </thead>
                      ...
                      <td className="px-5 py-4 align-middle">
                        {i === 0 && (
                          <div className="flex flex-col gap-1">
                            ...
                            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wide tabular-nums">{yama.startLabel} – {yama.endLabel}</span>
                        ...
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className={`w-6 h-6 rounded-md flex items-center justify-center text-xs font-black transition-all shrink-0 ${isExp ? 'bg-amber-400 text-amber-950' : 'bg-slate-100 hover:bg-slate-200 text-slate-500'}`}
                            >
                              {isExp ? '−' : '+'}
                            </button>
                          </div>
                          {(hora || gowriSlot || warnings.length > 0) && (
                            <div className="flex items-center gap-1 flex-wrap">
                              {hora && (
                                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[11px] font-black uppercase tracking-wide bg-slate-100 text-slate-600">
                              ...
                              {gowriSlot && (
                                <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[11px] font-black uppercase tracking-wide bg-slate-50 ${GOWRI_TEXT[gowriSlot.type.nature] || 'text-slate-600'}`}>
                              ...
                              {warnings.map(w => (
                                <span key={w} className={`px-1.5 py-0.5 rounded text-[11px] font-black uppercase tracking-wide ${SPECIAL_BADGE[w].cls}`}>
                      ...
                      <tr className="bg-amber-50/40">
                        <td colSpan={7} className="px-5 py-0">
                          <div className="border-l-2 border-amber-300 ml-8 my-3 pl-4">
                            <p className="text-[11px] font-black uppercase tracking-widest text-amber-600 mb-2.5">{c.sookshima}</p>
                                    ...
                                    <tr key={j} className="border-t border-amber-100/60">
                                      ...
                                      <td className="py-2.5 text-sm font-bold text-slate-500 tabular-nums whitespace-nowrap">
                                        <div className="flex items-center gap-1.5 flex-wrap">
                                          ...
                                          {skGowri && (
                                            <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[11px] font-black uppercase tracking-wide bg-slate-50 ${GOWRI_TEXT[skGowri.type.nature] || 'text-slate-600'}`}>
        ...
        {yamas.map((yama) => (
          <div key={yama.index} className="space-y-px bg-slate-50/30">
            ...
            <div className="px-4 sm:px-6 py-2.5 sm:py-3 bg-white border-b border-slate-100 flex justify-between items-center gap-2">
               ...
               <span className="text-[11px] font-black text-indigo-600 bg-indigo-50 px-4 py-1.5 rounded-full uppercase tracking-tight">
                   ...
                   <div key={key} className="p-3 sm:p-4 bg-white/60">
                      <div className="flex justify-between items-start mb-2">
                         <div className="flex flex-col gap-1">
                            ...
                            <div className="flex items-center gap-1 flex-wrap">
                              <span className="text-[11px] font-bold text-slate-400 tabular-nums">{s.startLabel} – {s.endLabel}</span>
                              {mobHora && (
                                <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded text-[11px] font-black uppercase tracking-wide bg-slate-100 text-slate-600">
                                  <span className={`w-2 h-2 rounded-full shrink-0 ${PLANET_DOT[mobHora.planet.key] || 'bg-slate-400'}`} />
                                  {lang === 'ta' ? mobHora.planet.tamil : mobHora.planet.label}
                                </span>
                              )}
                              {mobGowri && (
                                <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded text-[11px] font-black uppercase tracking-wide bg-slate-50 ${GOWRI_TEXT[mobGowri.type.nature] || 'text-slate-600'}`}>
                                  <span className={`w-2 h-2 rounded-full shrink-0 ${GOWRI_DOT[mobGowri.type.nature] || 'bg-slate-400'}`} />
                                  {lang === 'ta' ? mobGowri.type.tamil : mobGowri.type.label}
                                </span>
                              )}
                              {mobWarnings.map(w => (
                                <span key={w} className={`px-2 py-1 rounded text-[11px] font-black uppercase tracking-wide ${SPECIAL_BADGE[w].cls}`}>
                                  {SPECIAL_BADGE[w][lang]}
                                </span>
                              ))}
                            </div>
                      ...
                      <p className="text-sm text-slate-500 leading-normal italic px-1">
                        {s.palan || '—'}
                      </p>
                      ...
                      {isExp && (
                        <div className="mt-4 p-4 rounded-2xl bg-amber-50/50 border border-amber-100 animate-in slide-in-from-top-2">
                           <p className="text-[11px] font-black uppercase tracking-widest text-amber-600 mb-3.5">{c.sookshima}</p>
                                  ...
                                  <div key={j} className="flex justify-between items-start">
                                     <div className="flex flex-col gap-0.5">
                                        ...
                                        <div className="flex items-center gap-1 flex-wrap">
                                          ...
                                          {skGowriMob && (
                                            <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded text-[11px] font-black uppercase tracking-wide bg-slate-50 ${GOWRI_TEXT[skGowriMob.type.nature] || 'text-slate-600'}`}>
                                              <span className={`w-2 h-2 rounded-full shrink-0 ${GOWRI_DOT[skGowriMob.type.nature] || 'bg-slate-400'}`} />
